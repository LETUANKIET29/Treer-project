 ...
 * @author Triệu
 */
public class addNewPlantAuctionServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (!nameplant.isEmpty()) {
                AuctionPlantDao.AddNewAuctionPlant(nameplant, description, imgpath);
                request.setAttribute("MSG", "Cây đã được thêm vào");
            } else {
                request.setAttribute("MSG", "Cây chưa được thêm vào");
            }
            request.getRequestDispatcher("auctionPlantManagerServlet").forward(request, response);
        }
    }
